  ...
  emailSignup: asyncHandler(async (req, res, next) => {
    try {
      ...
      if (match) {
        res
          .status(404)
          .json({ status: false, message: "user already registerd" });
      } else {
        ...
        if (response == null) {
          res
            .status(401)
            .json({ status: false, id: "", message: "Enter correct email" });
        } else {
          res
            .status(200)
            .json({ status: true, id: user._id, message: "OTP sent" });
        }
      }
    } catch (error) {
  ...
  emailVerifyOtp: asyncHandler(async (req, res) => {
    ...
    await User.findById({ _id })
      ...
      .catch((err) => {
        res
          .status(401)
          .json({ status: false, message: "please check otp", token: "" });
      });
  }),
      ...
      if (findUser) {
        ...
        if (match) {
          if (findUser.user_isVerified) {
            ...
            res
              .status(401)
              .json({ status: false, message: "User n't verified", token: "" });
          }
        } else {
          res
            .status(401)
        ...
        res
          .status(401)
          .json({ status: false, message: "User n't registerd", token: "" });
      }
    } catch (error) {
      res
        .status(401)
        .json({ status: false, message: error.message, token: "" });
    }
  }),
    ...
    try {
      ...
      if (result == "verification") {
        ...
        if (findUser) {
          res
            .status(200)
            .json({ status: true, _id: findUser._id, message: "OTP sent" });
        } else {
          const user = User({
          ...
          res
            .status(200)
            .json({ status: true, _id: user._id, message: "OTP sent" });
        }
      } else {
        res
          .status(401)
          .json({ status: false, _id: "", message: "Enter Correct Number" });
      }
    } catch (error) {
  ...
  verifyMobile: asyncHandler(async (req, res) => {
    try {
      ...
      if (response === "approved") {
        ...
        res
          .status(401)
          .json({ status: false, token: "", message: "Check OTP" });
      }
    } catch (error) {
      res
        .status(401)
        .json({ status: false, token: "", message: error.message });
    }
  }),
};
